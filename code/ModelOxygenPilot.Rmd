...
media <- data[which(data$Oxygen == "Medium"),]
...
all.oxygen <- plot(jitter(rep(1, length(media$Concentration)), amount = 0.1),
      media$Concentration, ylim = c(-0.1, 10), xlim = c(0.5, 2.5), pch = 21, 
...
arrows(x0 = c(1), y0 = oxygen.table$mean, y1 = oxygen.table$`95LL`, angle = 90,
       length = 0.1, lwd = 2)

arrows(x0 = c(1), y0 = oxygen.table$mean, y1 = oxygen.table$`95UL`, angle = 90,
       length=0.1, lwd = 2)

mtext(expression('Oxygen Concentration (mg/L)'), side = 2,
