...
source(here("code", "EcoPlate", "EcoPlate.R"))

file.path <- here("data", "Villi_Pilot", "EcoPlate")

file.names <- list.files(path = file.path, all.files=FALSE,
                         full.names=FALSE, ignore.case=FALSE, include.dirs=FALSE)

file.names <- file.names[grep("*_VP_*", file.names)]
...
resource.names <- as.matrix(read.table(here("code", "EcoPlate", "resource_matrix.txt")))
mol.groups <- as.matrix(read.delim(here("code", "EcoPlate", "moleculetype_matrix.txt"), header=F))
...
colnames(eco.data) <- c("Sample", "Set", "Hours", resources, "NumRes")
eco.data$Sample <- strains
eco.data$Set <- sets
...
for (i in strains){
  ...
  for (j in resources){
    data.r <- which(eco.data$Sample  == i)
...
write.table(eco.data, here("data", "Villi_Pilot", "EcoPlate", "eco.data_VP.txt"), quote=FALSE,
            row.names=FALSE, sep="\t")
