...
Buffer <- ggplot(subset(buffer, Buffer == "MOPS"), aes(x = log(Dilution, 10), y = pH))+
  ...
  geom_point(aes(group = Inoculum, color = Inoculum), shape = 4)+
  geom_point(subset(buffer, Buffer == "Potassium Phosphate"), mapping = aes (x = log(Dilution, 10), y = pH, group = Inoculum, color = Inoculum))+
  xlab("log(buffer(M))")+
  ylab("pH")+
  ylim(c(4, 9))+
  xlim(c(-12, 0))
